...
const buildAst = (firstObject, secondObject) => {
  ...
  return keys.map((key) => {
    ...
    if (oldValue && newValue) {
      return (lodash.isObject(oldValue) && lodash.isObject(newValue)) ?
        { key, newValue, oldValue: buildAst(oldValue, newValue), type: 'nochanged' } :
        { key, newValue, oldValue, type: 'changed' };
    }
